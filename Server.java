...
public class Server extends Application { //Extends Application to implement JavaFX features
	...
	static Semaphore mutex = new Semaphore(1); // Creating mutuex with single access semaphore referred from https://www.mkyong.com/java/java-thread-mutex-and-semaphore-example/
			...
			public void run() {
				try {
					...
					while (true) { //Below lines will loop forever as to ensure the availability of Server all the time
						...
						waitList.add(waitTime);// Adding the integer sent by client to waitList
						...
						mutex.acquire();//Entering into critical section by acquiring mutex 
						waitTime=waitList.remove(0);//Pop out the integer from the waitList
						...
						mutex.release(); //Exiting critical section by releasing mutex
					...
					Platform.runLater(new Runnable() {
						...
						public void run() {
							...
							mutex.release();// Releasing mutex in case client disconnected while in critical section
